...
const getNewsItemsRand = (items: ArticleListItemProps[], numOfItems: number) => {
  let art = items
    ?.filter((item) => item.name !== 'Data' && item.name !== 'Authors')
    .slice(0, numOfItems || undefined);
...
const ArticleListFakeDynamic = (props: ArticleListComponentProps): JSX.Element => {
  const id = props.params?.RenderingIdentifier;

  const [newsItems, setNewsItems] = useState<ArticleListItemProps[]>([]);
  useEffect(() => {
    let auxNewsItems = getNewsItemsRand(
      props.fields?.items,
      Number.parseInt(props.params?.NumberOfItems)
    );
    setNewsItems(auxNewsItems);
  }, []);
...
export const DynamicRecommendation =
  withDatasourceCheck()<ArticleListComponentProps>(ArticleListFakeDynamic);
